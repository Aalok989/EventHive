...
function Footer() {
  return (
    ...
    <ul className='md:w-full flex flex-row flex-wrap items-center justify-center gap-4'>
        ...
        <li key={link.title} className='border flex items-center justify-center rounded-full p-2 text-2xl hover:scale-125 transition-all'>
            <Link className='' target={'_blank'} to={link.link} title={link.title}>{link.icon}</Link>
        </li>
        )}
    </ul>
    <div>
        <p className='text-neutral-300 text-xs md:text-sm text-center'>&copy; EventHive {new Date().getFullYear()} | All rights reserved | <Link className='hover:text-accent hover:underline' target='_blank' to='https://github.com/Aalok989'>Aalok Kumar</Link></p>
    </div>
    </div>
  )
}
