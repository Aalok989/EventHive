...
function CreateEventLogic() {
  ...
  const getEventById = useCallback(async () => {
    try {
      ...
      // Parse location and meet data from JSON strings
      const locationData = JSON.parse(locationJson || '[]');
      const meetData = JSON.parse(meetJson || '[]');
      
      setLocation((prev) => locationData[0] || '');
      setLatitude((prev) => locationData[1] || '');
      setLongitude((prev) => locationData[2] || '');
      ...
      setMeetLink((prev) => meetData[0] || '');
      setMeetId((prev) => meetData[1] || '');
      setMeetPassword((prev) => meetData[2] || '');
    ...
    try {
      ...
      try {
        ...
        const value = {
          ...
          location: medium === "online" 
            ? JSON.stringify([])
            : JSON.stringify([String(location), String(latitude), String(longitude)]),
          meet: medium === "offline"
            ? JSON.stringify([])
            : JSON.stringify([String(meetLink), String(meetId), String(meetPassword)]),
          privacy,
